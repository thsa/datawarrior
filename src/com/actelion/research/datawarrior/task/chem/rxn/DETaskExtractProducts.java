...
public class DETaskExtractProducts extends DETaskAbstractFromReaction {
	public static final String TASK_NAME = "Extract Reaction Products";
	...
	public DETaskExtractProducts(DEFrame parent) {
	...
	public void processRow(int row, int firstNewColumn, StereoMolecule containerMol, int threadIndex) {
		...
		if (rxn != null && rxn.getProducts() != 0) {
			...
			for (int i=1; i<rxn.getProducts(); i++)
				products.addMolecule(rxn.getProduct(i));
			new CoordinateInventor().invent(products);
			Canonizer canonizer = new Canonizer(products);
